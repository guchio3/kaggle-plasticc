...
from tools.fold_resampling import get_fold_resampling_dict, haradasan_get_fold_resampling_dict
...
#    okumura_trn_df = pd.read_pickle(
#            './lcfit/LCfit_feature_train_v4_20181205.pkl.gz', compression='gzip')
#    train_df = train_df.merge(okumura_trn_df, on='object_id', how='left')
#    del okumura_trn_df
#    gc.collect()

    train_df = train_df[FEATURES_TO_USE + ['target']]

#    okumura_trn_df2 = pd.read_pickle(
#            './lcfit/train_v2_20181213.pkl.gz', compression='gzip')#[OKUMURA_2_BEST_FEATS]
#    train_df = train_df.merge(okumura_trn_df2, on='object_id', how='left')
#    del okumura_trn_df2
#    gc.collect()
...
#            logger.info('best_itr : {}'.format(lgb.best_iteration_))
        ...
        for trn_idx, val_idx in tqdm(list(folds)):
            ...
            fold_resampling_dict = \
                haradasan_get_fold_resampling_dict(
                    y_trn,
                    logger,
                    SAMPLING_LOWER,
                    SAMPLING_LOWER_RATE)
...
#                feval=wloss_metric,
            ...
            logger.info('best score : {}'.format(booster.best_score['valid_1']['multi_logloss']))
            ...
            conf_y_pred.append(np.argmax(val_pred_score_zeropad, axis=1))
...
#####                logger=logger)
            ...
            test_df = pd.read_feather('./features/test/meta_features.ftr', nthreads=args.nthread)#[list(set(test_df.columns.tolist()) & set(features_to_use)) + ['object_id']]
...
#            okumura_test_df = pd.read_pickle(
#                    './lcfit/LCfit_feature_test_v4_20181205.pkl.gz', compression='gzip')
#            test_df = test_df.merge(okumura_test_df, on='object_id', how='left')
#            del okumura_test_df
#            gc.collect()
...
#                    './lcfit/test_v2_20181213.pkl.gz', compression='gzip')#[OKUMURA_2_BEST_FEATS]
#            test_df = test_df.merge(okumura_test_df2, on='object_id', how='left')
#            del okumura_test_df2
#            gc.collect()
